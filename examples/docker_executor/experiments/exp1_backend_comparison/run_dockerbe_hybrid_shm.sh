#!/bin/bash
# Exp1 variant: DockerBE Hybrid SHM
# Host vLLM with DockerDistributedExecutor, SHM broadcast MQ, TCP response MQ,
# and ASYNC_OUTPUT_COPY=1.
# This isolates the effect of TCP vs SHM for the worker response path.
#
...
RESULTS_DIR="${RESULTS_DIR:-$SCRIPT_DIR/results/dockerbe_hybrid_shm}"
...
run_tp() {
    ...
    log "Starting DockerBE hybrid-SHM server (TP=$tp)"
    ...
    if ! wait_for_server "$SERVER_STARTUP_TIMEOUT"; then
        err "DockerBE hybrid-SHM server did not start (TP=$tp)"
