#!/bin/bash
# Exp1 variant: DockerBE Sync Output
# Host vLLM with DockerDistributedExecutor, full SHM MQs, ASYNC_OUTPUT_COPY=0.
# This isolates the cost of the synchronous output copy path.
#
...
RESULTS_DIR="${RESULTS_DIR:-$SCRIPT_DIR/results/dockerbe_sync_output}"
...
run_tp() {
    ...
    log "Starting DockerBE sync-output server (TP=$tp)"
    ...
    if ! wait_for_server "$SERVER_STARTUP_TIMEOUT"; then
        err "DockerBE sync-output server did not start (TP=$tp)"
