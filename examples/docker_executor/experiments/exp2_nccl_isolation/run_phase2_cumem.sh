#!/bin/bash
# Phase 2 Variant: DockerBE + CUMEM Isolation (private IPC, TCP MQs)
# Per-GPU worker containers with NCCL CUMEM P2P recovery.
# Uses TCP message queues because private IPC namespaces block POSIX SHM.
...
RESULTS_DIR="${RESULTS_DIR:-$PHASE2_RESULTS_DIR/vllm_dockerbe_cumem}"

main() {
    verify_prereqs
    print_header "DockerBE + CUMEM Isolation, TCP MQs (TP=$TP_SIZE)"
